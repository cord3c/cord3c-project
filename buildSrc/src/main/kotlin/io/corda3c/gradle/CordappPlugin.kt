...
import io.corda3c.gradle.SignJar.Companion.sign
import io.corda3c.gradle.Utils.Companion.compareVersions
 ...
 */
@Suppress("UNUSED")
public class CordappPlugin @Inject constructor(private val objects: ObjectFactory) : Plugin<Project> {
    ...
    private fun configureCordappJar(project: Project) {
        ...
        if (cordapp.shading) {
            ...
            jarTask = project.tasks.getByName("shadowJar") as Jar

            val assembleTask = project.tasks.getByName("assemble")
            assembleTask.dependsOn(jarTask)

            // project.tasks.create("cordappJar", Jar::class.java)
            // jarTask.baseName = "test"
            // jarTask.dependsOn("jar")
            // jarTask.from(project.configurations.getByName("runtime").asFileTree)
        } else {
            jarTask = project.tasks.getByName("jar") as Jar
        }
    ...
    private fun checkPlatformVersionInfo(): Pair<Int, Int> {
        ...
        val minimumPlatformVersion: Int = cordapp.minimumPlatformVersion ?:  1
        val targetPlatformVersion = cordapp.targetPlatformVersion
