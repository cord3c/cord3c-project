...
import io.cord3c.rest.server.internal.CordaMapperImpl;
...
public class VCNetworkMapConfiguration {
	...
	public CordaMapper cordaMapper(PartyToDIDMapper didMapper) {
		CordaMapper mapper = new CordaMapperImpl();
	...
	@Bean
	public ServletRegistrationBean partyDidServlet(NodeMapRepositoryImpl repository, NetworkMapResolverProperties properties,
			VCCrypto crypto) {
		String url = "/parties/*";
	...
	@Bean
	public NetworkParametersRepositoryImpl networkParametersRepository(NetworkMapClient networkMapClient,
			CordaMapper cordaMapper) {
		...
		try (CloseableHttpClient client = builder.build()) {
			...
			if (response.getStatusLine().getStatusCode() <= 299) {
				...
				throw new IllegalStateException(
						"failed to obtain network truststore from " + url + ", got " + response.getStatusLine().toString());
			}
		}
	}
}
